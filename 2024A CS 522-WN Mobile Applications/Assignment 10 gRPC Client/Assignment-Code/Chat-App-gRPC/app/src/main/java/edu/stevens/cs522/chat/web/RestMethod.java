...
public class RestMethod {
    ...
    public ChatServiceResponse perform(RegisterRequest request) {
        try {
            ...
            ChatServiceBlockingStub chatServiceBlockingStub = createClient(request.chatServer, request);
            chatServiceBlockingStub.register(registration);
            ...
            Log.e(TAG, "Registration: Web service error, status code = " + response.errorMessage);
            return response;
        }
    }
    ...
    public UploadObserver perform(SynchronizeRequest request, final DownloadObserver downloadObserver) {
        ...
        ChatServiceStub chatServiceStub = createStreamingClient(Settings.getServerUri(context), request);
        final StreamObserver<UploadItem> requestProducer = chatServiceStub.sync(responseConsumer);
        /*
        ...
        return new UploadObserver() {
            ...
            public void onSync(long lastSequenceNumber, Double longitude, Double latitude) {
                ...
                SyncRequest syncRequest = SyncRequest.newBuilder().setLocation(location).setVersion(lastSequenceNumber).build();
