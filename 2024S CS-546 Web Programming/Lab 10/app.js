...
app.use('/', (req, res, next) => {
        ...
        let authentication;
        if (!req.session.user) authentication = "(Non-authenticated user)"
        else if ((req.session.user.role === "admin" && req.originalUrl !== "/") || (req.session.user.role === "user" && method !== "/admin" && method !== "/")) 
        authentication = "(Authenticated User)";
        else authentication = "(Non-authenticated user)";
        console.log("[" + date + "]: " + method + " " + route + " " + authentication);
        next();
});
